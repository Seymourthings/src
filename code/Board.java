...
public class Board implements IBoard {
	...
	/**
	 * Constructor for the Board class. Creates a multi-dimensional array that
	 * represents the board used during gameplay. If the index is not in the
	 * required scope, it sets it to null, otherwise, the tile is set to '_'
	 * 
	 */
	public Board() {
		_x = this.getRandomNum();
		_y = this.getRandomNum();
		_turnLocations = new ArrayList<Integer>();
		_board = new Tile[20][20];
		Tile t = new Tile('_');
		for (int i = 0; i < 20; i++) {
			for (int j = 0; j < 20; j++) {
				if (! ((i < _y && j > (19 - _x)) || (i > (19 - _y) && j < _x))) {
					_board[i][j] = null;
	...
	public int getNullColms(){
		return _y;
	}
	
	public int randomLocations(){
		double num =   ((Math.random()) * 600);
		int number = (int) num;
		return number;		
	}
	...
	/**
	 * Place the Tile at the given location as long it is a valid move according
	 * to gaming procedures. The first int parameter corresponds to desired row
	 * while the second corresponds to the desired column. Returns true it is a
	...
	public boolean place(Tile tile, int row, int col) {
		...
		if (_board[row][col].getChar() == '_') {
			...
			switch (state) {
			case 0:
				if (hasUp(row, col) || hasDown(row, col) || hasLeft(row, col)
						|| hasRight(row, col)) {
					if (_turnLocations.size() > 3) {
						state = 2;
					} else {
						state = 1;
					}
			...
			case 2:
				if (row == _turnLocations.get(0)) {
					if ((col == _turnLocations.get(1) + 1)
							|| (col == _turnLocations.get(1) + 1)) {
						state = 3;
					}
				} else if (col == _turnLocations.get(1)) {
					if ((row == _turnLocations.get(0) + 1)
							|| (row == _turnLocations.get(0) + 1)) {
						state = 3;
					}
				} else {
					return false;
	...
	/**
	 * Removes the desired Tile from the board as long as it was played during
	 * the current player's turn. Returns true if executed successfully, false
	 * otherwise.
	 * 
	 * @param Tile
	 * @return boolean
	...
	/**
	 * Gives a string representation of the current state of the board. '_'
	 * represents an empty square, ' ' represents a null and character
	 * otherwise.
	 * 
	 * @return String.
	 * 
	...
	/**
	 * Ends the turn of the current player and resets the record of moves they
	 * made.
	 */
	public void endTurn() {
		_turnLocations = new ArrayList<Integer>();
	...
	/**
	 * Returns true if the given location has a Tile to the left of it. False
	 * otherwise.
	 * 
	 * @return boolean
	 */
	...
	/**
	 * Returns true if the given location has a Tile to the right of it. False
	 * otherwise.
	 * 
	 * @return boolean
	 */
